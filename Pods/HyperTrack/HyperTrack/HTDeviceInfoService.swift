...
class HTDeviceInfoService: NSObject {
    ...
    func getLastKnownInfo(infoType : String?) -> String?{
        var info = HypertrackUserDefaults.standard.string(forKey: infoType!)
        if(info == nil){
            info = ""
        }
        return info
    ...
    func saveLastKnownInfo(infoType : String  , value : String ){
        HypertrackUserDefaults.standard.set(value, forKey: infoType)
    ...
    func onBatteryChanged(){
        ...
        DDLogInfo("battery status : " + batteryLevelString )
    ...
    func onNetworkStateChanged(state: String,isConnected: Bool){
        ...
        DDLogInfo("device.radio.changed :" + state)
        if(Settings.getUserId() != nil && Settings.getTracking()){
            let event = HyperTrackEvent(
                userId:Settings.getUserId()!,
                recordedAt:Date(),
