 ...
 class RequestManager {
    ...
    func flushEvents(completionHandler: @escaping (_ error: HyperTrackError?) -> Void){
        if !isPostingEvents {
            self.isPostingEvents = true
            DDLogInfo("flushing events")
            ...
            if let error = self.getPublishableKey().1 {
                self.isPostingEvents = false
                DDLogError("flush events fails as publishable Key is not set : " + error.description)
            ...
            HTTPRequest(method:.post, urlPath:"sdk_events/bulk/", arrayParams:eventsDict).makeRequest { response in
                ...
                case .success:
                    ...
                    DDLogInfo("Events flushed successfully: \(String(describing: eventIds.count))")
                       // Flush data
                      self.flushEvents(completionHandler: completionHandler)
                case .failure(let error):
                    DDLogError("Error while flushEvents: \(String(describing: error))  with response: \(String(describing: response))")
                    ...
                    let htError = HyperTrackError(HyperTrackErrorType.unknownError)
