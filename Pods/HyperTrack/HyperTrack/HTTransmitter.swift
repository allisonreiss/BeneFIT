...
final class Transmitter {
    ...
    func initialize() {
        ...
        if self.isTracking {
            DDLogInfo("started tracking as the tracking was initialized previously")
    ...
    func startTracking(completionHandler: ((_ error: HyperTrackError?) -> Void)?) {
        ...
        if  HTUserService.sharedInstance.getUserId() != nil &&   HTUserService.sharedInstance.getUserId() != "" {
    ...
    func stopTracking(completionHandler: ((_ error: HyperTrackError?) -> Void)?) {
        ...
        PushNotificationService.deleteRegisteredToken()
    ...
    func refreshTransmitter() {
        ...
        DDLogInfo("Set new transmitter controls, batch duration: \(String(describing: batchDuration)), displacement: \(String(describing: minimumDisplacement))")
    ...
    @objc func resetTransmitter() {
        ...
        DDLogInfo("Resetting transmitter controls as ttl is over")
