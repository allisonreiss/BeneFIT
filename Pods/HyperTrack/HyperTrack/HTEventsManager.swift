...
class EventsManager : NSObject {
    ...
    override init() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2, execute: {
            self.checkForDeviceInfoChangeEvent()
        })
    }
    
    func checkForDeviceInfoChangeEvent(){
        var isChanged = false
        if let info = HypertrackUserDefaults.standard.object(forKey: "device.info.changed") as? [String:Any]{
            if let sdkVersion = info["sdk_version"] as? String {
                if sdkVersion != Settings.sdkVersion{
                    isChanged = true
                }
            }
        }
...
extension EventsManager {
    ...
    func saveDeviceInfoChangedEvent() {
        ...
        let info = HTGenericUtils.getDeviceInfo()
        let event = HyperTrackEvent(
            userId:userId,
            recordedAt:Date(),
            eventType:eventType,
        ...
        HypertrackUserDefaults.standard.set(info, forKey:"device.info.changed")
