...
class HTActionService: NSObject {
    ...
    func createAndAssignAction(_ actionParams:HyperTrackActionParams, _ completionHandler: @escaping (_ action: HyperTrackAction?, _ error: HyperTrackError?) -> Void) {
        ...
        var action = [
            ...
            "collection_id":actionParams.collectionId as Any
    ...
    func completeAction(actionId: String?) {
        ...
        if(actionId != nil){
            ...
            let userInfo = ["actionId" as AnyHashable : actionId as Any]
    ...
    func completeActionInSynch(actionId: String,completionHandler: @escaping (_ action: HyperTrackAction?, _ error: HyperTrackError?) -> Void) {
        let params = [String : Any]()
       
        DDLogInfo("flushing events before completing action in synch")
        self.requestManager.flushEvents { (error) in
            if error != nil {
                DDLogError("all events are not flushed")
            }
            
            self.requestManager.completeActionInSynch(actionId, params,completionHandler:completionHandler)
        }
    }
    
    func completeActionWithLookupId(lookupId: String?,completionHandler: @escaping (_ action: HyperTrackAction?, _ error: HyperTrackError?) -> Void) {
        guard let userId = Settings.getUserId() else { return }
        ...
        let event = HyperTrackEvent(userId: userId, recordedAt: Date(), eventType: "action.completed", location: Settings.getLastKnownLocation(), data: ["lookup_id": lookupId])
        event.save()
        self.requestManager.postEvents()        
    }
    
    func patchExpectedPlaceInAction(actionId: String,newExpectedPlaces:HyperTrackPlace,_ completionHandler: @escaping (_ action: HyperTrackAction?, _ error: HyperTrackError?) -> Void) {
    ...
    func trackActionFor(collectionId: String, completionHandler: ((_ actions: [HyperTrackAction]?, _ error: HyperTrackError?) -> Void)?) {
        HTMap.sharedInstance.trackActionFor(collectionId: collectionId, completionHandler: completionHandler)
    }
    ...
    func removeActionForCollectionId(collectionId : String, clearMap:Bool = true){
        HTMap.sharedInstance.removeActions(clearMap:clearMap)
        if (collectionId == HTConsumerClient.sharedInstance.trackedUserStore?.trackedUserDataSource.collectionId){
            HTConsumerClient.sharedInstance.trackedUserStore?.trackedUserDataSource.collectionId = nil
        }
    }
