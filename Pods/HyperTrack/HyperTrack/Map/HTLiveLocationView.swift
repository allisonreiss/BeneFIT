...
@objc public enum LocationSelectionType: Int {
    case MY_LOCATION = 0
    case ON_MAP = 1
    case FROM_SEARCH_RESULTS = 2
    case FROM_SAVED_RESULTS = 3
    case UNKNOWN = 4
}
...
class HTLiveLocationView: HTCommonView {
    ...
    var savedPlaces : [HyperTrackPlace]? = nil
    var locationSelectionType = LocationSelectionType.FROM_SEARCH_RESULTS
    
    func initMapView(mapProvider:MapProviderProtocol,interactionViewDelegate: HTViewInteractionInternalDelegate) {
        self.mapProvider = mapProvider
        let view = self.mapProvider?.getMapView()
//        self.liveLocationView.addSubview(view!)
        self.mapView = view
        self.liveLocationView.insertSubview(view!, at: 0)
        self.interactionViewDelegate = interactionViewDelegate
        self.refocusButton.isHidden = true
        if HTConsumerClient.sharedInstance.getCollectionId() != nil {
    ...
    @IBAction func onConfirmLocationButtonClick(_ sender: Any) {
        if self.selectedLocation != nil {
            didSelectedLocation(place: self.selectedLocation!,selectionType: locationSelectionType)
    ...
    override func confirmLocation()-> HyperTrackPlace?{
        if let location = self.selectedLocation{
            didSelectedLocation(place: location, selectionType: locationSelectionType)
        }
        ...
        if(currentViewState == LiveLocationViewState.TRACKING_STARTED){
            if let userAnnotation = currentUserAnnotation {
                if let location = Transmitter.sharedInstance.locationManager.getLastKnownLocation(){
                    DDLogInfo("updating user coordinate" + location.description)
        ...
        if let mapView = self.mapView as? MKMapView {
            if let view = mapView.view(for: annotation) {
                if let markerView = view.subviews.first as? HTMarkerWithTitle {
                    ...
                    DDLogInfo("headingDirection " + (headingDirection?.description)!)
    ...
    func didSelectedLocation(place : HyperTrackPlace, selectionType:LocationSelectionType){
        ...
        if self.destinationAnnotation != nil {
            self.mapProvider?.removeMarker(annotation : self.destinationAnnotation!)
        }
        ...
        mapProvider?.addMarker(annotation: mapAnnotation)
        self.destinationAnnotation = mapAnnotation
        
        self.interactionViewDelegate?.didSelectLocation?(place: place,selectionType: selectionType)

        if(!(selectionType == LocationSelectionType.MY_LOCATION || selectionType ==  LocationSelectionType.ON_MAP)){
            ...
            if let location = Transmitter.sharedInstance.locationManager.getLastKnownLocation(){
                ...
                currentLocationAnnotation.type = MarkerType.HERO_MARKER
                currentLocationAnnotation.image =  UIImage.init(named: "purpleArrow", in:  Settings.getBundle(), compatibleWith: nil)
                mapProvider?.addMarker(annotation: currentLocationAnnotation)
                self.focusMarkers()
    ...
    func focusMarkers(force : Bool = false){
        if(!self.didUserStartPanning || force){
            ...
            mapProvider?.updateViewFocus(mapEdgePadding: mapEdgePadding)
    ...
    override func resetDestinationMarker(_ actionIdToBeUpdated: String?, showExpectedPlacelocation:Bool) {
        ...
        if(currentViewState == LiveLocationViewState.TRACKING_STARTED){
            ...
            if let selectedLocation = self.selectedLocation{
                ...
                self.destinationAnnotation?.type = MarkerType.DESTINATION_MARKER
                self.destinationAnnotation?.action = action
                
                if(!isAdded){
                    self.mapProvider?.addMarker(annotation: self.destinationAnnotation!)
                }
            }
    ...
    func animateMarker(userId: String,
        ...
        if let heroAnnotation = (self.mapViewDataSource?.getMapViewModel(userId: userId)?.heroMarker){
            ...
            if let view = view {
                ...
                if (heroAnnotation.type == MarkerType.HERO_MARKER_WITH_ETA) {
                    ...
                    if(currentIndex < coordinates.count - 1) {
                        ...
                        if let markerView = view?.subviews.first as? HTMarkerWithTitle {
                            if (disableHeroMarkerRotation == false) {
                                ...
                                }else if let adjustedHeading = user?.expandedUser?.lastLocation?.bearing {
                                    markerView.markerImage.transform = CGAffineTransform(rotationAngle: CGFloat(adjustedHeading * Double.pi / 180.0))
...
extension HTLiveLocationView : UITableViewDelegate{
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
            ...
            if let location = Transmitter.sharedInstance.locationManager.getLastKnownLocation() {
                getSearchResultsForCoordinate(cordinate: location.coordinate, completionHandler: { place, error in
                    if(error == nil && place != nil){
                        if indexPath.row == 0{
                            self.locationSelectionType = LocationSelectionType.MY_LOCATION
                            self.didSelectedLocation(place: place!,selectionType: LocationSelectionType.MY_LOCATION)
                        }else{
                            self.locationSelectionType = LocationSelectionType.ON_MAP
                            self.didSelectedLocation(place: place!,selectionType: LocationSelectionType.ON_MAP)
                        }
                    }
                })
            }
        ...
        if(isShowingSavedResults){
            ...
            locationSelectionType = LocationSelectionType.FROM_SAVED_RESULTS
            self.didSelectedLocation(place: location,selectionType: LocationSelectionType.FROM_SAVED_RESULTS)

        }else{
            location = (searchResults?[indexPath.row])!
            locationSelectionType = LocationSelectionType.FROM_SEARCH_RESULTS
            self.didSelectedLocation(place: location,selectionType: LocationSelectionType.FROM_SEARCH_RESULTS)
...
extension HTLiveLocationView : UITableViewDataSource{
    ...
    func getSavedPlaces() -> [HyperTrackPlace]?{
        if savedPlaces == nil {
            savedPlaces =  Settings.getAllSavedPlaces()
        }
        return savedPlaces
    }
            ...
            if let annotation = self.destinationAnnotation {
                ...
                DispatchQueue.main.async {
                    ...
                    self.getSearchResultsForCoordinate(cordinate: annotation.coordinate, completionHandler: { place, error in
                        ...
                        self.pinnedImageView?.removeFromSuperview()
                        self.mapProvider?.addMarker(annotation: annotation)
